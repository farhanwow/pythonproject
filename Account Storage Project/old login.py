...
class firstuse(object):
    ...
    def __init__(self, first):
        top = self.top1 = Toplevel(first)
        top.title('Input New Password')
        top.geometry('{}x{}'.format(250, 100))
        top.resizable(width=False, height=False)
        self.l = Label(top, text=" Password : ", font=('Courier', 14), justify=CENTER)
        self.l.pack()
        self.f1 = Entry(top, show='*', width=30)
        self.f1.pack(pady=7)
        self.f = Button(top, text='Submit', command=self.firstup, font=('Courier', 14))
        self.f.pack()

    def firstup(self):
        self.value1 = self.f1.get()
...
class entity_display:
    ...
    def __init__(self, master, n, e, p, i):
        ...
        for letter in dencryptedP:
            if dencryptedP == ' ':
                cdencryptedP += '*'
            else:
                cdencryptedP += '*'
        
        self.label_name = Label(self.window, text=dencryptedN, font=('Times New Roman', 14))
        self.label_email = Label(self.window, text=dencryptedE, font=('Comic Sans MS', 14))
        self.label_pass = Label(self.window, text=cdencryptedP, font=('Calibiri', 14))
...
def onshowabout():
    ons = Label(window, text = 'made by khan')
    ons.grid(row=1, column=4, padx=2, pady=2, sticky=W)
...
reveal = Button(window, text='Reveal Password', command=onreveal, font=('Courier', 14))
showabout = Button(window, text='About', command=onshowabout, font=('Courier', 14))
...
submit.grid(columnspan=3, row=4, column=0, padx=2, pady=2, sticky=W)
reveal.grid(columnspan=3, row=4, column=2, padx=2, pady=2, sticky=W)
showabout.grid(columnspan=3, row=0, column=3, padx=2, pady=2, sticky=W)
